...
void title_sequence_from_json(json_t* json, title_sequence* sequence);
title_command title_sequence_command_from_json(json_t* json, title_sequence* sequence);
...
void title_sequence_open(const char *path, const char *customName)
{
    ...
    if (preset == gConfigTitleSequences.num_presets)
    {
        ...
        if (customName == NULL)
        {
            ...
            size_t folderNameEnd = String::LastIndexOf(nameBuffer->GetString(), *PATH_SEPARATOR);
            // Remove everything after that
            nameBuffer->SubString(0, folderNameEnd);
            // Find the separator before it
            size_t folderNameStart = String::LastIndexOf(nameBuffer->GetString(), *PATH_SEPARATOR);
            // Remove everything before that
            nameBuffer->SubString(folderNameStart + 1, nameBuffer->GetLength());
            Memory::CopyArray<char>(gConfigTitleSequences.presets[preset].name, nameBuffer->GetString(), TITLE_SEQUENCE_NAME_SIZE);
            gConfigTitleSequences.presets[preset].path[0] = '\0';
            delete nameBuffer;
        }
        else
